...
N_cens <- sum(y_obs$y_value > 340)
y_obs_cencerd <- y_obs %>% mutate( y_value = if_else(y_value > 340, 340, y_value))
...
fit <- stan(file = "model/censored_data.stan",
            data = stan_data,
            iter = 2000,
            chains = 4,
            seed = 1234,
...
y_obs_censored_estimated <- y_obs_censored_estimated$y_value[340 >= y_obs_censored_estimated$y_value]
...
y_obs_over340 <- data.frame(y_value = y_obs$y_value[y_obs$y_value > 340])

ggplot(data = y_estimated,aes(x = y_estimated, color ="r")) + 
      geom_density() + geom_density(data = y_obs_over340,
