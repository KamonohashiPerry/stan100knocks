...
dataset <- dataset %>% mutate(tarsus = if_else(tarsus > 0, 1, 0),
                              back = if_else(back > 0, 1, 0))
...
stan_data <- list(N = nrow(dataset), # // num row
                  D = ncol(dataset %>% select(tarsus,back)), # // num vector
                  K = ncol(dataset %>% select(hatchdate,sex)), # // num regressor
                  x = as.matrix(dataset %>% select(-tarsus,-back)), # // regressor
                  y = as.matrix(dataset %>% select(-hatchdate,-sex)) # // vector
)

fit <- stan(file = "model/multivariate_probit_regression.stan",
            data = stan_data,
            iter = 2000,
            chains = 4,
            seed = 1234,
