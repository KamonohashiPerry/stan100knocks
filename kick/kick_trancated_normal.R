...
y_obs_trunc <- y_obs %>% filter(y_value < 330)


# Kick Stan model ---------------------------------------------------------
# https://mc-stan.org/docs/2_18/stan-users-guide/truncated-data-section.html

stan_data <- list(
                  N = nrow(y_obs_trunc),
                  U = 330,
                  y = as.matrix(y_obs_trunc$y_value)[,1]
)


fit <- stan(file = "model/trancated_normal.stan",
            data = stan_data,
            iter = 2000,
            chains = 4,
            seed = 1234,
...
param_names <- c('mcmc', paste0('mu'), paste0('sigma'))
d_est <- data.frame(1:N_mcmc, as.numeric(ms$mu), as.numeric(ms$sigma))
